    ...
    <>
      ...
      <Main>
        ...
        <Box>            
              ...
              onAaddWatched={handalAddWatchedList}
              watched={watched}/>
              ):(
...
function MoviDetails({selectedid,onclosemovie,onAaddWatched,watched})
{
  ...
  return(
   ...
   {!IsWached ? ( <div><StarRating maxRating={10} size={24} onSetRating={setUserRating}/>
    { 
    userRating > 0 
    &&
...
function WatchedMoviesList({ watched,ondeleteWatch}) {
  return (
    <ul className="list">
      {watched.map((picture) => (
        <WatchedMovie movie={picture} key={picture.imdbID}  ondeleteWatch={ondeleteWatch}/>
      ))}
    </ul>
  );
}
